...
class PlayerProfile {
    ...
    async saveModel() {
        try {
            const saveResult = await this.model.save('localstorage://playerModel');
    ...
    async loadModel() {
        try {
            ...
            if (modelData) {
                this.model = await tf.loadLayersModel('localstorage://playerModel');
                console.log('Model loaded from localStorage');
            }
            if (trainingData) {
                this.trainingData = JSON.parse(trainingData);
                console.log('Training data loaded:', this.trainingData.inputs.length, 'samples');
            }
        } catch (error) {
            console.error('Error loading model:', error.message);
            this.initModel(); // Fallback to new model
        }
    }
