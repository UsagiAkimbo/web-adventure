...
class PlayerProfile {
    ...
    recordSample(type, value) {
        if (this.tutorialState === 'active') {
            ...
            if (step && !step.complete) {
                this.samples[`${type}s`].push(value);
            ...
            this.samples[`${type}s`].push(value);
            if (this.samples[`${type}s`].length > 100) {
                this.samples[`${type}s`].shift(); // Keep last 100 samples
            }
            this.updateProfile();
        }
    }
    ...
    async updateProfile() {
        // Use TensorFlow.js k-means for clustering
        const features = {
            shoulderDistances: tf.tensor2d(this.samples.shoulderDistances.map(d => [d])),
            punchVelocities: tf.tensor2d(this.samples.punchVelocities.map(v => [v])),
            punchElbowAngles: tf.tensor2d(this.samples.punchElbowAngles.map(a => [a])),
            walkingCadences: tf.tensor2d(this.samples.walkingCadences.map(c => [c]))
        };
