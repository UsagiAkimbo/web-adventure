...
class PlayerProfile {
    constructor() {
        ...
        this.isTraining = false; // Training lock
        this.trainingQueue = []; // Queue for training requests
        this.lastTrainTime = 0; // Debounce timer
    ...
    async trainModel() {
        if (this.trainingData.inputs.length < 10) {
            this.trainingQueue = []; // Clear queue if not enough data
            return;
        }
        if (this.isTraining) {
            this.trainingQueue.push(true); // Queue request
            return;
        }
        ...
        try {
            ...
            if (this.trainingQueue.length > 0) {
                ...
                this.trainModel(); // Process next in queue
            }
        }
    }
    ...
    recordSample(type, value) {
        ...
        // Debounce training to every 5 seconds
