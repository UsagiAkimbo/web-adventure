...
class MotionCapture {
    constructor(player, profile) {
        ...
        this.wireframeCtx = this.wireframeCanvas.getContext('2d', { willReadFrequently: true });
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    setTimeout(() => reject(new Error('Video metadata timeout')), 10000); // Increased timeout
                });
    ...
    async processFrames() {
        ...
        try {
            if (this.videoElement.videoWidth <= 0 || this.videoElement.videoHeight <= 0) {
                console.warn('Skipping frame processing: invalid video dimensions, retrying...');
                setTimeout(() => requestAnimationFrame(() => this.processFrames()), 1000); // Retry after 1s
                return;
            }
    ...
    combineResults() {
        ...
        const keypointMap = {
            5: 11, 6: 12, 11: 23, 12: 24, 13: 25, 14: 26, 7: 13, 8: 14, 9: 15, 10: 16
        };
        ...
        if (leftShoulder && rightShoulder && (leftShoulder.x !== 0 || leftShoulder.y !== 0) && (rightShoulder.x !== 0 || rightShoulder.y !== 0)) {
            ...
            if (this.distanceSamples.length < this.sampleCount) {
                ...
                if (this.useShoulders && distanceRatio < this.distanceThreshold) {
                    ...
                    const intensity = Math.min((1 - distanceRatio) * 2, 2);
    ...
    processPunch(handPos, handLabel, punchRadius) {
        for (const resource of this.player.resources) {
            if (handPos.distanceTo(resource.position) < punchRadius) {
                ...
                resource.userData.spin = { startTime: performance.now(), duration: 1000 };
                this.profile.awardXP('combat', 10);
            }
        }
        for (const enemy of this.player.enemies) {
            if (handPos.distanceTo(enemy.position) < punchRadius) {
                ...
                this.profile.awardXP('combat', 20);
            }
        }
        for (const animal of this.player.animals) {
            if (handPos.distanceTo(animal.position) < punchRadius) {
                ...
                this.profile.awardXP('combat', 15);
            }
        }
        for (const portal of this.player.portals) {
            if (handPos.distanceTo(portal.position) < punchRadius) {
                console.log(`Punched portal to ${portal.userData.href} with ${handLabel} hand!`);
                document.getElementById('urlInput').value = portal.userData.href;
                document.querySelector('#controls button[onclick="generateEnvironment()"]').click();
                this.profile.awardXP('exploration', 50);
            }
        }
    }
