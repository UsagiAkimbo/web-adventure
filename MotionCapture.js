    ...
    processPunch(handPos, handLabel, punchRadius) {
        ...
        for (const enemy of this.player.enemies) {
            if (handPos.distanceTo(enemy.position) < punchRadius) {
                console.log(`Punched enemy with ${handLabel} hand at (${enemy.position.x.toFixed(2)}, ${resource.position.y.toFixed(2)}, ${resource.position.z.toFixed(2)})!`);
        ...
        for (const portal of this.player.portals) {
            if (handPos.distanceTo(portal.position) < punchRadius) {
                ...
                this.profile.awardXP('exploration', 50); // XP for using portal
            }
        }
    }
