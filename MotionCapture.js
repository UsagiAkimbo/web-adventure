...
class MotionCapture {
    constructor(player) {
        ...
        this.baselineShoulderDistance = 0;
        this.distanceSamples = [];
        this.sampleCount = 10; // Samples for baseline
        this.distanceThreshold = 0.9; // 10% reduction to trigger turn
    ...
    async stopMotionTracking() {
        ...
        // Reset shoulder distance tracking
        this.baselineShoulderDistance = 0;
        this.distanceSamples = [];
    }
    ...
    combineResults() {
        ...
        // Shoulder distance-based turning
        ...
        if (leftShoulder && rightShoulder && (leftShoulder.x !== 0 || leftShoulder.y !== 0) && (rightShoulder.x !== 0 || rightShoulder.y !== 0)) {
            ...
            if (this.distanceSamples.length < this.sampleCount) {
                ...
                if (distanceRatio < this.distanceThreshold) {
                    // Shoulders are closer; determine direction
                    ...
                    // Turn based on which shoulder moves more
                    if (Math.abs(leftShoulderXDelta) > Math.abs(rightShoulderXDelta) && leftShoulderXDelta > 0) {
                        // Left shoulder moves right (x increases), turn right
                        this.player.targetYaw -= (1 - distanceRatio) * 0.1; // Scale rotation by distance reduction
                    } else if (Math.abs(rightShoulderXDelta) > Math.abs(leftShoulderXDelta) && rightShoulderXDelta < 0) {
                        // Right shoulder moves left (x decreases), turn left
                        this.player.targetYaw += (1 - distanceRatio) * 0.1;
                    }
                }
            }
