...
class EnvironmentGenerator {
    constructor(canvas) {
        ...
        this.character = null;
        this.characterSpeed = 1.0; // Faster movement
        this.characterYaw = 0;
        this.targetYaw = 0; // For smoothing
        this.velocity = new THREE.Vector3(0, 0, 0); // For smooth movement
        this.lastStep = 0;
        ...
        this.lastStepTime = 0;
        this.stepCooldown = 300; // Faster steps
    ...
    async startMotionTracking() {
        ...
        try {
            ...
            document.getElementById('videoFeed').srcObject = stream; // Sync video feed
            this.videoElement.play();
            document.getElementById('videoFeed').play();
            console.log('Webcam stream started:', stream.active);
    ...
    generate(data) {
        ...
        if (!data.isAquatic) {
            ...
            this.terrain = null; // No land in aquatic mode
        }

        // Water terrain (<table> or aquatic)
        const waterSize = data.isAquatic ? 50 : 10; // Full scene or lake
        ...
        const waterColor = data.isAquatic ? 0x3399ff : 0x0000ff; // Brighter for aquatic
        const waterMaterial = new THREE.MeshBasicMaterial({ color: waterColor, opacity: 0.5, transparent: true });
        this.water = new THREE.Mesh(waterGeometry, waterMaterial);
        this.water.rotation.x = -Math.PI / 2;
        if (!data.isAquatic) {
            this.water.position.set((Math.random() - 0.5) * 30, 0, (Math.random() - 0.5) * 30); // Random lake position
        }
        ...
        for (let i = 0; i < data.animals; i++) {
            const typeIndex = data.isAquatic ? 1 : Math.min(Math.floor(data.animalTypeFactor / 50), 2); // Fish in aquatic
